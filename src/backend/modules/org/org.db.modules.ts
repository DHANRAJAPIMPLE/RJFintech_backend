...
export class OrgStructureDbController {
  ...
  private static extractImpactSummary(data: any): OrgImpactSummary {
    ...
    return {
      userAccess: Array.isArray(summary?.userAccess) ? summary.userAccess : [],
      workflow: Array.isArray(summary?.workflow) ? summary.workflow : [],
    };
  }
  ...
  private static async resolveImpactSummary(
    ...
    const hasNewFormat = Array.isArray(summary?.userAccess) && Array.isArray(summary?.workflow);

    if (hasNewFormat) {
      return {
        userAccess: summary.userAccess,
        workflow: summary.workflow,
      };
    }

    if (reqType === 'INITIATE') {
      const requestedNodePath = OrgStructureDbController.resolveRequestedNodePath(reqData);
      const userAccess = await OrgStructureDbController.getPropagatedUserAccessNames(
        client,
        companyId,
        requestedNodePath,
      );
      const workflow = await OrgStructureDbController.getAutoGeneratedWorkflowTemplateNames(
        client,
        companyId,
      ...
      const notification = await OrgStructureDbController.getOrgInactivationNotification(
        client,
        companyId,
        nodePath,
      );
      return {
        userAccess: notification.userNames,
        workflow: notification.workflowNames,
      };
    }
  }
  ...
  private static async getPropagatedUserAccessNames(
    client: any,
    companyId: string,
    newNodePath: string | null,
  ): Promise<string[]> {
    ...
    const names = parentAccesses
      .map((access: any) => access.user?.name)
  ...
  private static async getAutoGeneratedWorkflowTemplateNames(
    client: any,
    companyId: string,
    parentNodePath: string | null | undefined,
  ): Promise<string[]> {
    ...
    try {
      ...
      const requestData = {
        ...
        impactSummary: OrgStructureDbController.buildImpactSummary(
          impactSummaryNotification.userNames,
          impactSummaryNotification.workflowNames,
        ),
      };
          ...
          const updated = await tx.orgStructureReq.update({
            ...
            data: {
              ...
              data: {
                ...
                nodePath: resolvedNodePath,
                impactSummary: OrgStructureDbController.buildImpactSummary(
                  Array.from(new Set(newAccesses.map((acc: any) => acc.userName).filter((name: unknown): name is string => typeof name === 'string' && name.trim().length > 0))).sort() as string[],
                  Array.from(new Set(autoGeneratedWorkflowNotifications.map((w: any) => w.workflowName).filter((name: unknown): name is string => typeof name === 'string' && name.trim().length > 0))).sort() as string[],
                ),
              } as any,
              remarks,
            },
  ...
  static async initiateRequest(
    ...
    try {
      ...
      const request = await prisma.$transaction(async (tx) => {
        ...
        const propagatedAccessNames =
          await OrgStructureDbController.getPropagatedUserAccessNames(
            tx,
            resolvedCompanyId,
            requestedNodePath,
          );
        const autoGeneratedWorkflowNames =
          await OrgStructureDbController.getAutoGeneratedWorkflowTemplateNames(
            tx,
            resolvedCompanyId,
            reqData.parentNode?.nodePath,
          );
        const reqRecord = await tx.orgStructureReq.create({
          data: {
            ...
            type: 'INITIATE',
            impact: OrgStructureDbController.formatUserAccessImpact(
              propagatedAccessNames.length,
            ),
            data: {
              ...(reqData || {}),
              nodePath: requestedNodePath || reqData?.nodePath || null,
              impactSummary: OrgStructureDbController.buildImpactSummary(
                propagatedAccessNames,
                autoGeneratedWorkflowNames,
              ),
            },
    ...
    try {
      ...
      const pendingWithDetails = await Promise.all(
        pendingRequests.map(async (req) => {
          ...
          const resolvedImpactSummary = await OrgStructureDbController.resolveImpactSummary(
            prisma as any,
            resolvedCompanyId,
            req.type || 'INITIATE',
            req.data,
      ...
      if (inactiveImpactByNodePath) {
        ...
        for (const request of inactiveRequests) {
          ...
          if (
            ...
            const resolved = await OrgStructureDbController.resolveImpactSummary(
              prisma as any,
              resolvedCompanyId,
              'UPDATE',
              requestData,
            );
            inactiveImpactByNodePath.set(targetNodePath, resolved);
          }
        }
      }
      ...
      const safeNodes = visibleNodes.map((node) => ({
        ...
        isAutoDeleted: node.status !== 'ACTIVE',
        impactSummary: inactiveImpactByNodePath?.get(node.nodePath) ?? { userAccess: [], workflow: [] },
