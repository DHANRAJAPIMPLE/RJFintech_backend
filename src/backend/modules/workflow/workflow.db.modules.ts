...
export class WorkflowDbController {
  ...
  private static buildWorkflowTree(
    ...
    const buildNode = (row: any): any => {
      const autoGenerated = collectDescendants(row, new Set<string>([row.id]));

      return {
        ...row,
        isPending: pendingWorkflowIds.has(row.id),
        pendingRequest: row.pendingRequest ?? null,
        autoGenerated,
      };
    };
  ...
  static async fetchWorkflows(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const generatedWorkflowRows = await prisma.workflowReq.findMany({
        where: {
          companyId: resolvedCompanyId,
          type: 'AUTO_GENERATE',
          status: 'APPROVED',
      ...
      const autoGeneratedParentByWorkflowId = new Map<string, string>();
      generatedWorkflowRows.forEach((request) => {
        ...
        if (
          ...
          autoGeneratedParentByWorkflowId.set(request.workflowId, parentId);
        }
      });
      ...
      if (type === 'active' || type === 'inactive') {
        ...
        const visibleWorkflowRows = await prisma.workflow.findMany({
          where: type === 'active' ? activeBaseWhere : inactiveBaseWhere,
          select: activeSelect,
        });
