...
export class WorkflowDbController {
  ...
  static async fetchWorkflowHistory(
    ...
    try {
      ...
      if (typeof historyLookupId === 'string' && historyLookupId) {
        ...
        if (directRequest) {
          ...
          if (directRequest.workflowId) {
            const exactWorkflowRequestIds =
              await WorkflowDbController.getExactWorkflowRequestIds(
                prisma,
                resolvedCompanyId,
                directRequest.workflowId,
                nodeAccessFilter,
              );
            exactWorkflowRequestIds.forEach((requestId) =>
      ...
      const formattedHistories = histories.map((h) => {
        const requestType = String(h.workflowReq?.type || 'INITIATE').toUpperCase();
        ...
        const changeCount = snapshots.newData
          ? WorkflowDbController.getWorkflowHistoryChangeCountFromSnapshots(
              snapshots.oldData,
              snapshots.newData,
              h.workflowReq?.type,
  ...
  static async getWorkflowHistoryDetail(
    ...
    try {
      ...
      const requestType = String(history.workflowReq?.type || 'INITIATE').toUpperCase();
      const displayEvent = WorkflowDbController.getWorkflowHistoryDisplayEvent(
        history.event,
        requestType,
      );
