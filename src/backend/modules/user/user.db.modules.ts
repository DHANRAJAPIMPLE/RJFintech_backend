...
export class UserDbController {
  ...
  private static async buildWorkflowFilterDropdowns(
    ...
    const visibleWorkflows = workflows
      .filter((workflow) => {
        return !autoGeneratedParentByWorkflowId.has(workflow.id);
      })
      .map((workflow) => {
    ...
    const filteredWorkflows = normalizedFilters
      ? workflowRows.filter((workflow) => {
        ...
        if (
          !matchesTextFilter(
            normalizedFilters.currentStatus,
            workflow.status,
            workflow.status === 'ARCHIVE' ? 'archive' : workflow.status,
          )
        ) {
          return false;
        }
      ...
      const statusLabel =
        workflow.status === 'ACTIVE'
          ...
          : workflow.status === 'INACTIVE'
            ? 'Inactive'
            : workflow.status === 'ARCHIVE'
              ? 'Archive'
              : workflow.status === 'PENDING'
                ? 'Pending'
                : UserDbController.humanizeFilterLabel(workflow.status) ||
                  workflow.status;
      const statusKey = statusLabel.toLowerCase();
      const existingStatus = statusCounts.get(statusKey);
    ...
    return {
      ...
      currentStatus: Array.from(statusCounts.values()).sort((a, b) =>
        a.value.localeCompare(b.value),
      ),
      nodes,
    };
  ...
  private static isAutoGeneratedAccess(access: any, allAccesses: any[]) {
    if (access?.source === 'AUTO_GENERATED') return true;
    if (access?.source === 'USER') return false;
  ...
  private static async finalizeApprovedUserInitiation(
    ...
    defaultWorkflows.forEach((wf: any) => {
