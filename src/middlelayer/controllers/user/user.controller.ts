  ...
  static async initiateUserOnboarding(
    ...
    try {
      ...
      if (requestType !== 'initiate') {
        ...
        const autoGeneratedAccessNames = Array.isArray(
          (createRes as any)?.autoGeneratedAccessNames,
        )
          ? ((createRes as any).autoGeneratedAccessNames as string[])
          : null;

        return res.status(201).json({
          message:
            createRes?.message || 'User onboarding initiated successfully',
    ...
    try {
      ...
      const nodes: UserCompanyNode[] = rawNodes.map((node) => ({
        ...
        workflows: node.workflows.map((workflow) => ({
          ...
          selected: workflow.selected === true,
        })),
